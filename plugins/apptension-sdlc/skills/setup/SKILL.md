...
| Board | human-supplied. The checklist's `board` probe (step 2) only establishes that the owner has at least one GitHub Project — it ties none of them to this repo, learns no field IDs, and does not list views. Carry over an existing `### Dev flow bindings` section's value if the repo has one and that value already names a board view; a carried-over row that names a project and no view is resolved at the gate like a fresh one. Otherwise the row is `unknown` until a human names the GitHub Project, however many the probe returned. Once a project is named, the Board row records a `BOARD_LAYOUT` view of it — see "The Board row names a view" below. Under a Jira tracker this row is expected to be absent, and that is not a gap: `Board` is a GitHub Projects row, and Jira status transitions carry the card instead. |
| Task orchestrator | an existing `### Dev flow bindings` section's value, carried over verbatim, and nothing else. With no such row it is `unknown` and the gate asks. **Nothing on this machine is evidence for this row** — see below. `unknown` is not a failure: it means this repo works its issues in live sessions, which is the ordinary case. See the `cezar-setup` skill. |
...
Read the bundled `checklist.json`, next to this skill's own `SKILL.md` —
the manifest of repo artifacts these processes depend on, collected from
the `requires:` front-matter of every skill in this plugin. If the file
is absent — a partial copy of this skill, without the checklist bundled
beside it — **stop** and say the audit needs the full `apptension-sdlc`
plugin or skill directory installed: the checklist ships next to this
file and cannot be reconstructed from it. Read each entry's `intent`, `grants`, and `source` off that
file and render them verbatim; do not rewrite them from memory. Run each entry's `detect`
probe against the target repo and record one of `present`, `candidate`,
`missing`, or `unknown`.
...
[the prerequisites reference](../../references/prerequisites.md) instead.
...
session, by installing it — see
[the prerequisites reference](../../references/prerequisites.md) for
...
probe can supply — and a project title or view name carried over from an
example is a binding that points at somebody else's project.
...
### The Board row names a view, not only the GitHub Project

A GitHub Project holds several views, and more than one of those can be
a board (`BOARD_LAYOUT`). The `board` probe lists projects, not views, so
a row that records only the project leaves a reader unable to tell which
board the repo uses.

The row keeps the project identity `dev-flow` already reads — title,
owner, number — and adds the view. Status field IDs stay on the GitHub
Project: `gh project item-edit` does not take a view, and `dev-flow`
still moves cards that way.

How the view is chosen — recorded without asking when the named project
has exactly one board, asked when it has more than one — is the gate in
step 5.
...
`dev-flow`, and it is the only row that gets one, so
...
Bodies follow the `issue-authoring` skill — the same
...
`dev-flow` run can recover which skill this issue is an instance
of, without putting an internal name on screen in a repo where it
means nothing. Rendered markdown hides the whole line.
...
the `issue-intake` and `sdlc-docs-check` skills.
...
one line each, before the approve prompt. `Board` also asks when a
carried-over row names a GitHub Project and no board view — skip the
project question in that case and resolve the view as below:
...
After a Board answer names a GitHub Project — or a carried-over row
already names one and no view — resolve the view before the approve
prompt. Do not pick a project from `gh project list` to get here:
however many the probe returned, the project is still the human's to
name.

List its views. Query both roots — a user-owned project makes
`organization` null, and an org-owned project makes `user` null — and
read whichever `projectV2` is non-null:
...
Keep only nodes whose `layout` is `BOARD_LAYOUT`. A table or roadmap
view is not a board. Then:

| `BOARD_LAYOUT` views | What to do |
|---|---|
| none | Leave the row `unknown`. Say the named project has no board view. |
| exactly one | Write that view into the row. Do not ask. |
...
An answer that matches one of the listed views fills the row. Any other
answer, or none, leaves the whole Board row `unknown`. Never write a
project without a view.

A lookup that fails — wrong number, no access — leaves the row
`unknown`. Do not invent a view.
...
unbounded — a GitHub Project, a stack, a branch — so they have to be typed.
...
either: an internal skill name like `issue-authoring` means
nothing in a repo that has never seen this plugin, so it reads as a
loose end rather than a reference.

What a later `dev-flow` run needs to recover the entry is in the
first-line marker instead, where rendered markdown hides it:

```
<!-- repo-setup:area-labels source:issue-authoring -->
```
...
[the prerequisites reference](../../references/prerequisites.md) that
step 2 found unavailable,
...
named this as its third effect, so it is already approved. Run the
`cezar-setup` skill, which is where the procedure lives and stays.
...
the record of what this repo chose, and the `cezar-setup` skill says the
same thing when asked to configure it. Likewise `none`: it is an answer, and nothing is configured
or offered.
...
The test is whether the existing configuration equals what the
`cezar-setup` skill's procedure would write now — so
...
is exercised by `dev-flow` step 7 and by CI on the
