...
file" and the `--allowedTools` binding below for why):
`.sdlc-pr-context/meta.json` holds the title, body, and metadata, and
`.sdlc-pr-context/diff.patch` holds the diff. Read those two files
...
using the `Edit` tool — the only file-modifying tool this skill is
permitted, scoped to exactly this path. Treat overwriting the file as the
goal of the run, not cleanup at the end of a longer analysis; but a run
that never gets there is meant to fail rather than pass silently, so
there is no need to race a hasty verdict over a genuine crash.
...
The workflow (see the `pr-checks` skill for the sibling CI-side process)
...
| Process map | `docs/sdlc/overview.md` in the repo checkout — a site page, not bundled with this plugin |
| `claude-code-action` marketplace | `https://x-access-token:${{ github.token }}@github.com/apptension/toolkit-dev.git` — private repo, so the URL needs an embedded token; see the `pr-checks` skill's identical caveat |
...
CI — see the `pr-checks` skill's identical caveat. The
