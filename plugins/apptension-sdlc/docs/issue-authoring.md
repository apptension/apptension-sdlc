---
title: Authoring a well-formed issue
area: issue-intake
summary: How to draft a GitHub issue against task.yml so it reads as shipped work, not a planning exercise.
...
- **One name per thing.** Pick the repo's name for a file, a command, or
  a concept, then repeat it. Never alternate `marketplace.yaml`, "the
  manifest", and "the config", because the variation reads as a second
  thing.
...
- **This does not cover the SDLC docs' own markdown source.** This file,
  and every other doc under `docs/sdlc/`, stays wrapped at its existing
  width for readability in an editor. The rule is about the artifact an
  agent writes, not the doc that teaches it.
